...
protected:
    std::string ToStringExtra() const override {
        if (m_leaf_version.has_value()) {
            return strprintf("%s,%x", HexStr(m_leaf_script), m_leaf_version.value());
        }
        return HexStr(m_leaf_script);
    }
...
public:
    RawLeafDescriptor(CScript leaf_script, std::optional<int> leaf_version = std::nullopt) : DescriptorImpl({}, "rawleaf"), m_leaf_script(leaf_script), m_leaf_version(leaf_version) {}
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    if (ctx == ParseScriptContext::P2TR && Func("rawleaf", expr)) {
        ...
        if (!Const(",", expr)) {
            ...
            return std::make_unique<RawLeafDescriptor>(leaf_script);
        }
...
std::unique_ptr<DescriptorImpl> InferScript(const CScript& script, ParseScriptContext ctx, const SigningProvider& provider)
{
            ...
            if (tree) {
                ...
                for (const auto& [depth, script, leaf_ver] : *tree) {
                    ...
                    if (!subdesc) {
                        ...
                        if (auto rawleaf = dynamic_cast<RawLeafDescriptor*>(subdesc.get())) {
                            auto ver = rawleaf->GetLeafVersion();
                        }
    ...
    if (ctx == ParseScriptContext::P2TR) {
        return std::make_unique<RawLeafDescriptor>(script);
    }
