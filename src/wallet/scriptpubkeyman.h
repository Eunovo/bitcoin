...
private:
    ...
    using ChangeLabelTweak = std::pair<bip352::SilentPaymentLabel, uint256>;

    TweakMap m_map_spk_tweaks GUARDED_BY(cs_desc_man);
    std::optional<ChangeLabelTweak> m_change_label_tweak GUARDED_BY(cs_desc_man);
    ...
    const ChangeLabelTweak& ChangeLabel() const EXCLUSIVE_LOCKS_REQUIRED(cs_desc_man);
