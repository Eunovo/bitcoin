...
BOOST_AUTO_TEST_CASE(bip352_send_and_receive_test_vectors)
{
    ...
    for (const auto& vec : tests.getValues()) {
        ...
        for (const auto& recipient : vec["receiving"].getValues()) {
            ...
            std::vector<std::pair<bip352::SilentPaymentLabel, uint256>> labels;
            // Always calculate the change key, whether we use labels or not
            auto [change_label, change_tweak] = bip352::CreateLabelTweak(scan_priv_key, 0);
            labels.emplace_back(std::move(change_label), change_tweak);
            // If labels are used, add them to the dictionary
            for (const auto& label : given["labels"].getValues()) {
                auto [label_pubkey, label_tweak] = bip352::CreateLabelTweak(scan_priv_key, label.getInt<int>());
                labels.emplace_back(std::move(label_pubkey), label_tweak);
            }
