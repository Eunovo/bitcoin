...
SilentPaymentsReceiver::SilentPaymentsReceiver(const CKey& scan_key, const CPubKey& spend_pubkey)
    : m_scan_key(scan_key), m_spend_pubkey(spend_pubkey)
{
...
SilentPaymentsDestination SilentPaymentsReceiver::BuildLabeledDestination(const SilentPaymentsLabel& label) const {
    ...
    auto dest{SilentPaymentsDestination::From(0, m_scan_key.GetPubKey(), labeled_spend_pubkey)};
    assert(dest);
    return *dest;
}
...
std::vector<SilentPaymentsOutput> SilentPaymentsReceiver::Scan(
