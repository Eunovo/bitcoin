...
SilentPaymentLabel::SilentPaymentLabel(const secp256k1_silentpayments_label& label) : m_label{std::make_unique<secp256k1_silentpayments_label>(label)} {}

SilentPaymentLabel::SilentPaymentLabel(SilentPaymentLabel&&) noexcept = default;
SilentPaymentLabel& SilentPaymentLabel::operator=(SilentPaymentLabel&&) noexcept = default;

SilentPaymentLabel::~SilentPaymentLabel() = default;
...
std::pair<SilentPaymentLabel, uint256> CreateLabelTweak(const CKey& scan_key, const int m) {
    ...
    assert(ret);
    return {SilentPaymentLabel{label_obj}, uint256{label_tweak}};
}
...
std::optional<std::vector<SilentPaymentOutput>> ScanForSilentPaymentOutputs(
    ...
    const std::vector<std::pair<SilentPaymentLabel, uint256>>& labels
) {
    bool ret;
    ...
    std::vector<const unsigned char *> tx_output_ptrs;
    std::vector<secp256k1_silentpayments_label_entry> label_entries;
    std::vector<const secp256k1_silentpayments_label_entry *> label_entry_ptrs;
    found_output_objs.reserve(tx_outputs.size());
    found_output_ptrs.reserve(tx_outputs.size());
    tx_output_ptrs.reserve(tx_outputs.size());
    label_entries.reserve(labels.size());
    ...
    for (size_t i = 0; i < tx_outputs.size(); i++) {
        ...
        tx_output_ptrs.push_back(tx_outputs[i].data());
    }

    for (const auto& [label_obj, label_tweak] : labels) {
        secp256k1_silentpayments_label_entry label_entry{};
        label_entry.label = label_obj.Get();
        std::copy(label_tweak.begin(), label_tweak.end(), label_entry.label_tweak);
        label_entries.push_back(label_entry);
        label_entry_ptrs.push_back(&label_entries.back());
    }
    ...
    for (size_t i = 0; i < n_found_outputs; i++) {
        ...
        memcpy(sp_output.output.data(), found_output_objs[i].output, 32);
        sp_output.tweak = uint256{found_output_objs[i].tweak};
        if (found_output_objs[i].found_with_label) {
            sp_output.label = SilentPaymentLabel{found_output_objs[i].label};
        }
