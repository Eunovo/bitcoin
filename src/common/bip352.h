...
class SilentPaymentLabel {
private:
    static constexpr unsigned int SIZE = 68;

    std::shared_ptr<secp256k1_silentpayments_label> m_label;

public:
    SilentPaymentLabel(secp256k1_silentpayments_label&& label);

    SilentPaymentLabel(const SilentPaymentLabel&) = default;
    SilentPaymentLabel& operator=(const SilentPaymentLabel&) = default;

    SilentPaymentLabel(SilentPaymentLabel&&) noexcept = default;
    SilentPaymentLabel& operator=(SilentPaymentLabel&&) noexcept = default;

    bool operator==(const SilentPaymentLabel& other) const;

    bool operator<(const SilentPaymentLabel& other) const;

    bool operator>(const SilentPaymentLabel& other) const;

    ~SilentPaymentLabel() = default;
