 ...
 * recipient's own change destination. Keeping the scan key, spend public key, and labels together
 * on one object (rather than passing them around separately) means they can never be mismatched
 * across calls.
 *
 * The change label is always registered automatically on construction; additional labels can be
 * registered with GenerateLabeledAddress().
 */
class SilentPaymentsReceiver {
private:
    std::map<SilentPaymentsLabel, uint256> m_labels;
    std::map<SilentPaymentsLabel, uint256>::const_iterator m_change_it;
    CKey m_scan_key;
...
public:
    SilentPaymentsReceiver(const CKey& scan_key, const CPubKey& spend_pubkey);
    ...
    /**
     * @brief Derive this recipient's own silent payments change destination.
     *
     ...
     */
    std::vector<SilentPaymentsOutput> Scan(const PrevoutsSummary& prevouts_summary, const std::vector<XOnlyPubKey>& tx_outputs) const;
};
}; // namespace bip352
