...
struct SilentPaymentLabel {
private:
    std::unique_ptr<secp256k1_silentpayments_label> m_label;

public:
    SilentPaymentLabel(const secp256k1_silentpayments_label& label);

    // Delete copy constructors
    SilentPaymentLabel(const SilentPaymentLabel&) = delete;
    SilentPaymentLabel& operator=(const SilentPaymentLabel&) = delete;

    // Default move constructors
    SilentPaymentLabel(SilentPaymentLabel&&) noexcept;
    SilentPaymentLabel& operator=(SilentPaymentLabel&&) noexcept;

    // Default destructor
    ~SilentPaymentLabel();

    const secp256k1_silentpayments_label& Get() const { return *m_label; }
     ...
     */
    V0SilentPaymentDestination GenerateLabeledAddress(const V0SilentPaymentDestination& recipient) const;
};

struct SilentPaymentOutput {
    XOnlyPubKey output;
    uint256 tweak;
    std::optional<SilentPaymentLabel> label;
 ...
 * @return std::pair<SilentPaymentLabel, uint256> The label public key and label tweak.
 *
 ...
 */
std::optional<std::vector<SilentPaymentOutput>> ScanForSilentPaymentOutputs(const CKey& scan_key, const PrevoutsSummary& prevouts_summary, const CPubKey& spend_pubkey, const std::vector<XOnlyPubKey>& output_pub_keys, const std::vector<std::pair<SilentPaymentLabel, uint256>>& labels);
}; // namespace bip352
